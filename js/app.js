...
function log(msg, cls = "") {
  // Always console log so you can debug even if UI log is missing
...
function setVaultUI() {
  setText("vaultBalance", money(state.vaultUSDC));

  const canUseVault = !!state.pubkey;

  setDisabled("depositBtn", !canUseVault);
  setDisabled("withdrawBtn", !canUseVault);

  setOpacity("depositBtn", canUseVault ? "1" : ".55");
  setOpacity("withdrawBtn", canUseVault ? "1" : ".55");
}
...
// ✅ Guaranteed boot (prevents “nothing works”)
